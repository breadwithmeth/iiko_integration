...
describe("IikoNomenclatureService", () => {
  it("loads all product pages and marks missing products deleted", async () => {
    ...
    const client = {
      post: vi
        .fn()
        .mockResolvedValueOnce({ totalCount: 101, products: Array.from({ length: 100 }, (_, index) => product(index)) })
        .mockResolvedValueOnce({ totalCount: 101, products: [product(100)] })
    };

    const result = await new IikoNomenclatureService(client as never).syncMenu("org-iiko");
