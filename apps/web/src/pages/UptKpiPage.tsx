...
import { BarChart3, Calendar, Download, TrendingUp, Users, ChevronDown, Target } from "lucide-react";
import { formatMoney } from "../api/client";
import { useUptKpi, type UptKpiResponse } from "../api/hooks";
...
function getUptClass(upt: number): string {
  if (upt >= 3) return "upt-high";
  if (upt >= 2) return "upt-medium";
  if (upt >= 1) return "upt-low";
  return "upt-none";
}

export function UptKpiPage() {
  ...
  const kpi = useUptKpi(dateFrom || undefined, dateTo || undefined);
  ...
  if (isLoading && !data) {
    return (
      <section className="page-panel">
        <h1>KPI: UPT (Units Per Transaction)</h1>
        <div className="loading">Загрузка...</div>
      </section>
    );
  }

  const summary = data?.summary ?? { totalOrders: 0, totalItems: 0, totalAmount: 0, upt: 0, avgCheck: 0, ordersByStatus: {} };
  ...
  return (
    <section className="page-panel">
      <div className="page-header">
        <h1><Target size={24} /> KPI: UPT (Units Per Transaction)</h1>
      ...
      <div className="stats-grid">
        ...
        <div className="stat-card">
          ...
          <div className="stat-info">
            ...
            <span className="stat-label">Всего товаров (шт.)</span>
          </div>
        </div>
        <div className="stat-card highlight">
          <Target size={24} />
          <div className="stat-info">
            <span className="stat-value upt-value">{summary.upt.toFixed(2)}</span>
            <span className="stat-label">UPT (товаров на заказ)</span>
          </div>
        </div>
        <div className="stat-card">
          <Users size={24} />
          <div className="stat-info">
            <span className="stat-value">{formatMoney(summary.avgCheck)}</span>
      ...
      <div className="upt-legend">
        <span className="upt-legend-item"><span className="upt-dot upt-high"></span> UPT ≥ 3.0 — Отлично</span>
        <span className="upt-legend-item"><span className="upt-dot upt-medium"></span> UPT 2.0–2.9 — Хорошо</span>
        <span className="upt-legend-item"><span className="upt-dot upt-low"></span> UPT 1.0–1.9 — Нормально</span>
        <span className="upt-legend-item"><span className="upt-dot upt-none"></span> UPT {"<"} 1.0 — Низко</span>
      </div>

      {/* Operator KPI Table */}
      <div className="operator-stats">
        <div className="table-header">
          <h2>KPI по операторам</h2>
            ...
            <tbody>
              {byOperator.map((op, index) => (
                <tr key={op.operator.id} className={index % 2 === 0 ? "even" : "odd"}>
                  ...
                  <td className="orders-count">{op.totalItems}</td>
                  <td className="upt-cell">
                    <span className={`upt-value ${getUptClass(op.upt)}`}>{op.upt.toFixed(2)}</span>
                  </td>
                  <td className="amount">{formatMoney(op.avgCheck)}</td>
...
function exportToCSV(
  data: UptKpiResponse["byOperator"],
  preset: PeriodPreset,
  dateFrom: string,
  dateTo: string
) {
  const headers = ["Оператор", "Email", "Заказов", "Товаров (шт.)", "UPT", "Ср. чек", "Сумма"];
  const rows = data.map((op) => [
    op.operator.name,
    op.operator.email,
    String(op.totalOrders),
    String(op.totalItems),
    op.upt.toFixed(2),
    String(op.avgCheck / 100),
    String(op.totalAmount / 100)
  ]);
  ...
  link.download = `upt-kpi-${periodSuffix}.csv`;
  link.click();
